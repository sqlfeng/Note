...
FUNCTION gath_tm()
   ...
   OPEN WINDOW gath_w AT p_row,p_col WITH FORM "czz/42f/p_dayeff" 
...
WHILE TRUE
   ...
   ON ACTION CONTROLP
      CASE
          WHEN INFIELD(tc_ser14)
            CALL cl_init_qry_var()
            LET g_qryparam.state = 'c'
            LET g_qryparam.form = "q_gen"
            CALL cl_create_qry() RETURNING g_qryparam.multiret
            DISPLAY g_qryparam.multiret TO tc_ser14
            NEXT FIELD tc_ser14
      END CASE
   ON ACTION locale
...
FUNCTION p_dayeff()
   DEFINE l_name VARCHAR(20),		# External(Disk) file name
     ...
     LET l_sql = "SELECT DISTINCT '','','',",
                 " tc_ser14,count(*),0,0",             #add by leo 110901
                 " FROM tc_ser_file",
                # " WHERE (tc_ser13='0' OR tc_ser13='W') AND ",
                 " WHERE tc_ser13 NOT in('2','3','4','5','6','7','8','G') AND ",
                 " tc_ser06 >= '", tc_ser06_b,
     ...
     FOREACH gath_c1 INTO sr.*
       ...
       SELECT count(*) INTO sr.tc_ser16_1 FROM tc_ser_file 
        WHERE tc_ser14=sr.tc_ser14
          AND tc_ser13 not in ('2','3','4','5','6','7','8','G')
          AND tc_seracti='Y'
       ...
       SELECT count(*) INTO sr.tc_ser16_3 FROM tc_ser_file 
        WHERE tc_ser14=sr.tc_ser14
          AND tc_ser13 not in ('2','3','4','5','6','7','8','G')
          AND tc_seracti='Y'
          AND (((tc_ser15-tc_ser06-(select count(*) from sme_file where sme01 between tc_ser06 and tc_ser15 and sme02='N')) <= 2) AND (tc_ser15 >=tc_ser06))   #add by liudong 140401
          #AND ((tc_ser15 <= (tc_ser06+2)) AND (tc_ser15 >=tc_ser06))   #add by leo 110901
          AND tc_ser06 BETWEEN tc_ser06_b AND tc_ser06_e
       #问题结案率
       SELECT count(*) INTO l_sum FROM tc_ser_file
         WHERE tc_ser14=sr.tc_ser14
          AND tc_ser13 not in ('2','3','4','5','6','7','8','G')
          AND tc_seracti='Y'
          AND tc_ser06 BETWEEN tc_ser06_b AND tc_ser06_e
       ...
       OUTPUT TO REPORT gath_rep(sr.*,l_sum)
