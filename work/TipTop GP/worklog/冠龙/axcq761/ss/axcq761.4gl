...
DEFINE g_tlfa      DYNAMIC ARRAY OF RECORD
                   ...
                   ima25          LIKE ima_file.ima25,        #xqs161101
                   ...
                   ima10_l        LIKE ima_file.ima10,
                   cdj08a         LIKE cdj_file.cdj08,
            ...
            IF cl_chk_act_auth() THEN
               ...
               CASE g_action_flag 
                  ...
                  WHEN 'page3'
                     ...
                     CALL cl_export_to_excel(page,base.TypeInfo.create(g_tlf),'','')
               END CASE
            END IF
...
#                  "   ORDER BY tlf01,tlf905,tlf906 "  
      ...
      LET l_sql = "SELECT tlf905,tlf906,tlf01,ima02,ima021,ima25,tlf19,occ02,NVL(SUM(tlf10*tlf60*tlf907),0),NVL(SUM(tlfc21*tlf907),0),",   #xqs161101 add ima25
                  "        0 wsaleamt,0 l_wsale_tlf21,0 l_rate,1 salerat,1 raterat,occ04,'' gen02,'' gem01,'' gem02,",
                  "        '' oma08,'' oab01,occ03,'' warea_name,ima131,tlf036,NVL(occ37,'N'),",
#                  "       NVL(SUM(tlfc221*tlf907),0),NVL(SUM(tlfc222*tlf907),0),NVL(SUM(tlfc2231*tlf907),0),NVL(SUM(tlfc2232*tlf907),0),",
#                  "       NVL(SUM(tlfc224*tlf907),0),NVL(SUM(tlfc2241*tlf907),0),NVL(SUM(tlfc2242*tlf907),0),NVL(SUM(tlfc2243*tlf907),0),tlf13,tlf66,NVL(azf08,'N'),' ' ogb01,0 ogb03,tlf907 ",      #FUN-CC0157 add ogb01,ogb03  #MOD-D90159 add tlf907
                  "       NVL(SUM(tlfc221*tlf907),0),NVL(SUM(tlfc222*tlf907),0),NVL(SUM(tlfc2231*tlf907),0),",
                  "       NVL(SUM(tlfc224*tlf907),0),NVL(SUM(tlfc2241*tlf907),0),NVL(SUM(tlfc2242*tlf907),0),NVL(SUM(tlfc2243*tlf907),0),NVL(SUM(tlfc2232*tlf907),0),tlf13,tlf66,NVL(azf08,'N'),' ' ogb01,0 ogb03,tlf907 ",      #FUN-CC0157 add ogb01,ogb03  #MOD-D90159 add tlf907
                  "  FROM ",cl_get_target_table(m_plant[l_i],'ima_file'),",",cl_get_target_table(m_plant[l_i],'tlf_file'),
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'occ_file')," ON tlf19=occ01 ",
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'azf_file')," ON tlf14=azf01 AND azf02='2' ",   
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'tlfc_file')," ON tlfc01 = tlf01 ",
      ...
      ELSE 
         #LET l_sql1 ="   AND (tlf13 = 'axmt670' OR tlf99 IS NOT NULL )"  #xqs160402 marked
         LET l_sql1 ="  AND (tlf13 = 'axmt670')" #xqs160402 add
      END IF
      IF g_oaz.oaz92='N' THEN  #MOD-E20136 add begin----- 
         LET l_sql2 ="   AND ",tm.wc CLIPPED,
                     ...
                     "   AND (tlf06 BETWEEN '",tm.bdate,"' AND '",tm.edate,"')",
                     "   AND tlf905 LIKE TRIM(smyslip)||'-%' AND smydmy1 ='Y' ",    #No.MOD-D30160
                     " GROUP BY tlf01,ima02,ima021,ima25,ima131,occ02,occ03,occ04,occ37,tlf036,tlf13,tlf19,tlf66,tlf905,tlf906,azf08,tlf907 ", #MOD-D90159 add tlf907 #xqs161101 add ima25
                     "   ORDER BY tlf01,tlf905,tlf906 "  
...
#wujie 130904 --end
      ...
      LET l_sql = "MERGE INTO axcq761_tmp o",
                 ...
                 #"  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 )",  #FUN-CC0157 mod                           #MOD-D90121 mark
                  "  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 AND (o.tlf905 = x.oma76 OR x.oma76 IS NULL))", #MOD-D90121 add
                  " WHEN MATCHED ",
                  " THEN ",
                  "   UPDATE ",
...
#FUN-D10022----------------add-------------end-----------------------        
   ...
   IF tm.c = 'Y' THEN
      ...
      CALL s_ckk_fill('','312','axc-461',g_ccz.ccz01,g_ccz.ccz02,g_prog,tm.type,l_num1+l_num3,l_wsaleamt,l_wsaleamt,0,0,0,0,0,0,0,l_msg,g_user,g_today,g_time,'Y')
           RETURNING g_ckk.*
      IF NOT s_ckk(g_ckk.*,'') THEN END IF
       CALL q761_huiru()  #add by dengsy170103
   END IF
...
DEFINE i     LIKE type_file.num5
   ...
   LET g_sql = "SELECT w1,wi,wb,wx,w2,w3,w4,w5,w6,wc,w7,wd,w8,we "    #FUN-D10022 add  #xqs161101 add wc
   FOR i=1 TO 4
      IF NOT cl_null(l_cb[i]) THEN 
         ...
         CASE l_cb[i]
            ...
            #FUN-D10022--mark--end--
            #FUN-D10022--add--str--
            WHEN '1' LET l_str = l_str," tlf01,a.ima02,a.ima021,a.ima25"   #xqs161101 add ima25
                     LET l_str1= l_str1," tlf01"
                     ...
                     LET g_sql = cl_replace_str(g_sql,'wx','a.ima25')
   ...
   LET g_sql = cl_replace_str(g_sql,"wx","''")   #xqs161101 add
   #FUN-D10022--add--end--
   ...
   FOREACH tlfa_curs INTO g_tlfa[g_cnt].* 
      ...
      SELECT ima10 INTO g_tlfa[g_cnt].ima10_l FROM ima_file
      WHERE ima01 = g_tlfa[g_cnt].tlf01a
      IF cl_null(g_tlfa[g_cnt].ima10_l) THEN 
        LET g_tlfa[g_cnt].ima10_l = ' '
      END IF 
           #SELECT tlf07 INTO l_tlf07 FROM tlf_file WHERE tlf026 = g_tlf[g_cnt].tlf026 AND
           #tlf027 = g_tlf[g_cnt].tlf027
           #IF cl_null(l_tlf07) THEN 
             #SELECT tlf07 INTO l_tlf07 FROM tlf_file WHERE tlf026 = g_tlf[g_cnt].tlf036 AND
           #tlf027 = g_tlf[g_cnt].tlf027
           #END IF 
           SELECT cdj08 INTO g_tlfa[g_cnt].cdj08a FROM cdj_file WHERE cdj06 = g_tlfa[g_cnt].tlf01a
           AND cdj02 = year(tm.edate) AND cdj03 = month(tm.edate)
           IF NOT cl_null(g_tlfa[g_cnt].cdj08a) THEN
             SELECT aag02 INTO g_tlfa[g_cnt].aag02a FROM aag_file WHERE aag01 = g_tlfa[g_cnt].cdj08a 
           END IF 
      LET g_cnt = g_cnt + 1
   END FOREACH
...
DEFINE l_tlf07         LIKE tlf_file.tlf07
DEFINE l_ima10         LIKE ima_file.ima10  #xiangjie 170724
   ...
   FOREACH tlf_curs INTO g_tlf_excel[g_cnt].*,g_tlfc[g_cnt].*,l_tlf13,l_tlf66,l_azf08,l_ogb01,l_ogb03,l_tlf907,l_rowno  #FUN-D10022 add l_tlf13,l_tlf66,l_azf08,l_rowno #FUN-CC0157 add ogb01,ogb03  ##MOD-D90159 add l_tlf907
      IF SQLCA.sqlcode THEN
         ...
         EXIT FOREACH
      END IF
      SELECT ima10 INTO l_ima10 FROM ima_file WHERE ima01 = g_tlf_excel[g_cnt].tlf01
      ...
      IF g_cnt <= 100000 THEN 
         ...
         LET g_tlf[g_cnt].tlf026 =  g_tlf_excel[g_cnt].tlf026 #xiangjie 170721
         LET g_tlf[g_cnt].tlf027 =  g_tlf_excel[g_cnt].tlf027
                   LET g_tlf[g_cnt].tlf01 =  g_tlf_excel[g_cnt].tlf01
                   LET g_tlf[g_cnt].ima02 =  g_tlf_excel[g_cnt].ima02
                   LET g_tlf[g_cnt].ima021 =  g_tlf_excel[g_cnt].ima021 
                   LET g_tlf[g_cnt].ima25 =  g_tlf_excel[g_cnt].ima25
                   LET g_tlf[g_cnt].tlf19 =  g_tlf_excel[g_cnt].tlf19
                   LET g_tlf[g_cnt].wocc02 =  g_tlf_excel[g_cnt].wocc02
                   LET g_tlf[g_cnt].tlf10 =  g_tlf_excel[g_cnt].tlf10
                   LET g_tlf[g_cnt].tlfc21 =  g_tlf_excel[g_cnt].tlfc21
                   LET g_tlf[g_cnt].wsaleamt =  g_tlf_excel[g_cnt].wsaleamt
                   LET g_tlf[g_cnt].l_wsale_tlf21 =  g_tlf_excel[g_cnt].l_wsale_tlf21
                   LET g_tlf[g_cnt].l_rate =  g_tlf_excel[g_cnt].l_rate
                   LET g_tlf[g_cnt].salerat =  g_tlf_excel[g_cnt].salerat
                   LET g_tlf[g_cnt].raterat =  g_tlf_excel[g_cnt].raterat
                   LET g_tlf[g_cnt].occ04 =  g_tlf_excel[g_cnt].occ04
                   LET g_tlf[g_cnt].gen02 =  g_tlf_excel[g_cnt].gen02
                   LET g_tlf[g_cnt].gem01 =  g_tlf_excel[g_cnt].gem01
                   LET g_tlf[g_cnt].gem02 =  g_tlf_excel[g_cnt].gem02
                   LET g_tlf[g_cnt].oma08 =  g_tlf_excel[g_cnt].oma08
                   LET g_tlf[g_cnt].oab01 =  g_tlf_excel[g_cnt].oab01
                   LET g_tlf[g_cnt].occ03 =  g_tlf_excel[g_cnt].occ03
                   LET g_tlf[g_cnt].warea_name =  g_tlf_excel[g_cnt].warea_name
                   LET g_tlf[g_cnt].ima131 =  g_tlf_excel[g_cnt].ima131
                   LET g_tlf[g_cnt].tlf036 =  g_tlf_excel[g_cnt].tlf036
                   LET g_tlf[g_cnt].occ37 =  g_tlf_excel[g_cnt].occ37
                   LET g_tlf[g_cnt].ima10 =  l_ima10
         LET l_ima131 = NULL 
         SELECT ima131 INTO l_ima131 FROM ima_file WHERE ima01 = g_tlf_excel[g_cnt].tlf01
         IF NOT cl_null(l_ima131) THEN 
           IF g_tlf_excel[g_cnt].oma08 MATCHES '[1]' THEN 
             SELECT oba11 INTO g_tlf[g_cnt].oba11 FROM oba_file WHERE oba01 = l_ima131
           ELSE 
             SELECT oba17 INTO g_tlf[g_cnt].oba11 FROM oba_file WHERE oba01 = l_ima131
           END IF 
           IF NOT cl_null(g_tlf[g_cnt].oba11) THEN 
             SELECT aag02 INTO g_tlf[g_cnt].aag02 FROM aag_file WHERE aag01 = g_tlf[g_cnt].oba11
           END IF 
         END IF #IF NOT cl_null(l_ima131) THEN 
         IF NOT cl_null(g_tlf[g_cnt].tlf19) THEN 
           SELECT tlf07 INTO l_tlf07 FROM tlf_file WHERE tlf026 = g_tlf[g_cnt].tlf026 AND
           tlf027 = g_tlf[g_cnt].tlf027
           IF cl_null(l_tlf07) THEN 
             SELECT tlf07 INTO l_tlf07 FROM tlf_file WHERE tlf026 = g_tlf[g_cnt].tlf036 AND
           tlf027 = g_tlf[g_cnt].tlf027
           END IF 
           SELECT cdj08 INTO g_tlf[g_cnt].cdj08 FROM cdj_file WHERE cdj06 = g_tlf[g_cnt].tlf01
           AND cdj14 = g_tlf[g_cnt].tlf19 AND cdj02 = year(l_tlf07) AND cdj03 = month(l_tlf07)
           IF NOT cl_null(g_tlf[g_cnt].cdj08) THEN
             SELECT aag02 INTO g_tlf[g_cnt].aag02_l FROM aag_file WHERE aag01 = g_tlf[g_cnt].cdj08 
           END IF 
         END IF #IF NOT cl_null(g_tlf[g_cnt].tlf19) THEN 
      END IF #xiangjie 170721
      LET g_cnt = g_cnt + 1
   END FOREACH
   ...
   LET g_rec_b = g_cnt
   #FUN-C80092--add--str--
   IF g_rec_b > 100000 AND (g_bgjob='N' OR g_bgjob is null) THEN
...
#str------- add by 170103
FUNCTION q761_huiru()
  DEFINE g_count1       LIKE type_file.num20
  DEFINE l_bdate,l_edate        LIKE type_file.dat

  SELECT trunc(tm.bdate,'mm')  INTO l_bdate FROM dual   #第一天
   #最后一天
  SELECT last_day(tm.bdate) INTO l_edate FROM dual 

  IF l_bdate<>tm.bdate OR l_edate<>tm.edate THEN 
     #CALL cl_err('','cxc-334',1)
     RETURN 
  END IF 

  
  SELECT count(*) INTO g_count1 FROM tc_cxc_file
  WHERE tc_cxc03=year(tm.bdate) AND tc_cxc04=month(tm.bdate) AND tc_cxc01='axcq761'
  IF g_count1>0 THEN 
     DELETE FROM tc_cxc_file WHERE tc_cxc03=year(tm.bdate) AND tc_cxc04=month(tm.bdate) AND tc_cxc01='axcq761'
  END IF
  INSERT INTO tc_cxc_file SELECT 'axcq761',tlf905,year(tm.bdate),month(tm.bdate),tlf01,tlf10,tlfc21,rownum 
  FROM axcq761_tmp
  IF SQLCA.sqlcode OR SQLCA.SQLERRD[3] = 0 THEN
     CALL cl_err3("ins","tc_cxc_file","","",SQLCA.sqlcode,"","",1) 
  ELSE 
    #CALL cl_err( '', 9062, 0 )
  END IF 
END FUNCTION 
#end------- add by dengsy170103