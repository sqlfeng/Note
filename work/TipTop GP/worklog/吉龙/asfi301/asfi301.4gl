...
FUNCTION i301_ui_default()
   ...
   #   CALL cl_set_act_visible("get_bom,to_po,apmt420", TRUE)
   #ELSE
   #   CALL cl_set_act_visible("get_bom,to_po,apmt420", FALSE)
   #END IF
   # add by lixwz 20170928 e
...
DEFINE l_js        LIKE type_file.num15_3
   ...
   CALL i301_show()
END FUNCTION
FUNCTION i301_get_bom()
  #DEFINE l_sfa03
  DEFINE l_btflg    LIKE type_file.chr1
  DEFINE l_minopseq LIKE type_file.num5
  DEFINE l_msg      STRING
	  IF g_sfb.sfb87 != 'Y' THEN
	  		CALL cl_err('工单未审核不能从从BOM取包材','!',1)
	  		RETURN
	  END IF
	  IF g_sfb.sfb04 >= 7 THEN
	  		CALL cl_err('工单已入库不能从从BOM取包材','!',1)
	  		RETURN
	  END IF
	  IF g_sfb.sfb02 = 11 THEN
	     LET l_btflg = 'N'
	  ...
	  END IF
	  CALL s_minopseq(g_sfb.sfb05,g_sfb.sfb06,g_sfb.sfb071) RETURNING l_minopseq
	  CALL s_cralc7(g_sfb.sfb01,g_sfb.sfb02,g_sfb.sfb05,l_btflg,
	            #g_sfb.sfb08,g_sfb.sfb071,'Y',g_sma.sma71,l_minopseq,g_sfb.sfb95)  #No.TQC-610003
	             g_sfb.sfb08,g_sfb.sfb071,'Y',g_sma.sma71,l_minopseq,'',g_sfb.sfb95)  #No.TQC-610003  #FUN-BC0008 mod
	   RETURNING g_cnt
     LET l_msg = '已导入',g_cnt CLIPPED,'条包材'
...
DEFINE l_pml  RECORD LIKE pml_file.*,
  ...
  DEFINE p_row,p_col  LIKE type_file.num5
   ...
   IF g_sfb.sfb87 != 'Y' THEN
  		CALL cl_err('工单未审核不能抛转请购单','!',1)
   		RETURN
   END IF
   IF g_sfb.sfb04 >= 7 THEN
  		CALL cl_err('工单已入库不能抛转请购单','!',1)
  		RETURN
   END IF
   LET l_cnt = 0
   SELECT COUNT(*) INTO l_cnt FROM sfa_file
  	 WHERE SFA01 = g_sfb.sfb01 AND SFAUD01 IS  NULL
  	       AND SFAUD09 IS  NOT NULL
   IF l_cnt = 0 THEN
   		CALL cl_err('没有需要抛转的料件','!','1')
   		RETURN
   END IF
   ...
   SELECT 'N',SFA26,SFA01,SFA08,SFA03,IMA02,SFA01||'_'||SFA02,SFA01,IMA25,IMA44,
   			  IMA44_FAC,SUM(SFA05),SFB13
		 FROM SFA_FILE
		 LEFT JOIN IMA_FILE ON IMA01   =   SFA03
     LEFT JOIN  SFB_FILE ON SFA01   =   SFB01
	 WHERE SFA01 = g_sfb.sfb01 AND SFAUD01 IS  NULL AND SFAUD09 IS  NOT NULL # 还未回写
		GROUP BY  SFA26,SFA01,SFA08,SFA03,IMA02,SFA01,SFA02,
						  SFA01,IMA25,IMA44,IMA44_FAC,SFB13
   ...
   DISPLAY ARRAY g_pml TO s_pml.* ATTRIBUTE(COUNT=g_cnt,UNBUFFERED)
      BEFORE DISPLAY
   END DISPLAY
      ...
      IF g_success='Y' THEN
         ...
         FOR i=1 TO g_pml.getLength()
            IF g_pml[i].x='Y' THEN
               ...
               UPDATE sfa_file SET sfaud01 = l_pmk.pmk01
               	 WHERE sfa01 = g_sfb.sfb01 AND sfa03 = g_pml[i].pml04
               	 		AND sfa08 = g_pml[i].sfa08
            END IF
         END FOR
         #回写请购单号和标志位逻辑end
         COMMIT WORK
   ...
   CLOSE WINDOW p300p_w
END FUNCTION
FUNCTION i301_apmt420()
  DEFINE p_cmd STRING
  DEFINE l_cnt LIKE type_file.num5
  DEFINE l_sfaud01 LIKE sfa_file.sfaud01

    LET g_sql ="SELECT DISTINCT sfaud01 FROM sfa_file ",
               " WHERE sfa01 = '",g_sfb.sfb01 CLIPPED ,"'"
    PREPARE i301_pmk FROM g_sql
    DECLARE pmk_curs  CURSOR FOR i301_pmk
    LET p_cmd = "apmt420 '/",g_sfb.sfb01 CLIPPED
    #FOREACH pmk_curs INTO l_sfaud01
    #    IF SQLCA.sqlcode THEN
    #        CALL cl_err('foreach:',SQLCA.sqlcode,1)
    #        EXIT FOREACH
    #    END IF
    #    LET p_cmd = p_cmd CLIPPED,'/',l_sfaud01
    #END FOREACH
    LET p_cmd = p_cmd CLIPPED,"'"," ''"," ''"
