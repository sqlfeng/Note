...
MAIN
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'   # If background job sw is off
      THEN CALL csfr010_tm()        # Input print condition
      ELSE CALL csfr010()              # Read data and create out-file
   END IF
...
#add by shijl 170321--str--
  ...
  #             "    AND ",tm.wc CLIPPED
    ...
    # add by lixwz 20170728 e
#add by shijl 170321--end--
   PREPARE csfr010_prepare1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
       CALL cl_err('prepare:',SQLCA.sqlcode,1)
       CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
       EXIT PROGRAM
   END IF
   DECLARE csfr010_curs1 CURSOR FOR csfr010_prepare1
  ...
  LET l_sql = " SELECT SUM(mst08) ",
   ...
   END IF
   PREPARE csfr010_s1 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('p1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
   END IF
   DECLARE csfr010_curs2 CURSOR FOR csfr010_s1
{
   LET l_sql = " SELECT SUM(imk09) ",
               " FROM imk_file  ",
               " WHERE imk01 = ? AND imk05 = ?  AND imk06 = ? ",
               " AND ",tm.wc2 CLIPPED
   PREPARE csfr010_s2 FROM l_sql
   IF SQLCA.sqlcode != 0 THEN
      CALL cl_err('p1:',SQLCA.sqlcode,1)
      CALL cl_used(g_prog,g_time,2) RETURNING g_time #No.FUN-690123
      EXIT PROGRAM
   END IF
   DECLARE csfr010_curs3 CURSOR FOR csfr010_s2
}
   LET g_pageno = 0
   FOREACH csfr010_curs1 INTO sr.*
      IF SQLCA.sqlcode != 0  THEN
  ...
  #    OPEN csfr010_curs3 USING sr.sfa03,l_sma51,l_sma52
  #               FETCH csfr010_curs3 INTO sr.imk09
  #               IF SQLCA.SQLCODE THEN
  ...
  #               CLOSE csfr010_curs3
   ...
   # 24 个月的数据
      FOR i = 1 TO 24
           ...
           CASE i
                WHEN 1
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s1
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 2
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s2
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 3
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s3
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 4
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s4
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 5
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s5
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 6
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s6
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 7
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s7
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 8
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s8
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 9
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s9
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 10
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s10
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 11
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s11
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 12
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s12
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 13
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s13
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 14
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s14
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 15
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s15
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 16
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s16
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 17
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s17
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 18
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s18
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 19
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s19
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 20
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s20
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 21
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s21
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 22
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s22
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 23
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s23
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
                ...
                WHEN 24
                 OPEN csfr010_curs2 USING sr.sfa03,sr.mss_v,l_begin,l_end
                 FETCH csfr010_curs2 INTO sr.s24
                 IF SQLCA.SQLCODE THEN
                     CALL cl_err("", SQLCA.SQLCODE, 0)
                      CLOSE csfr010_curs2
                     EXIT FOR
      ...
      END FOR
      INSERT INTO csfr010_tmp (sfa03,ima02,
...
#add by shijl 170321--str--
      ...
      LET l_sql=" SELECT distinct a.ima01,a.ima02,a.ima021,a.ima08,a.ima46,(nvl(a.ima48,0)+nvl(a.ima49,0)+nvl(a.ima491,0)+nvl(a.ima50,0)), ",
                ...
                " ,imk_file b ",
                " LEFT JOIN csfr010_tmp c ON c.sfa03=b.imk01 AND b.imk02=c.imk02 ",
        ...
        LET sr.ccc92=sr.ccc91 *sr.ccc23
         ...
         FOREACH cxcr611_tlf INTO l_tlf.*
            ...
            ELSE
               ...
               LET l_check = "Y"
            END IF
            CALL csfr010_month(l_tlf.tlf06) RETURNING l_mm
         ...
         IF (l_check = 'N' AND sr.ccc91 > 0) THEN ##完全没有库存进料时,直接取呆滞日
            CALL csfr010_month(sr.ima902) RETURNING l_mm
...
#add by shijl 170321--end--
   ...
   IF g_zz05 = 'Y' THEN
      ...
      LET g_str = tm.wc
   END IF
  CALL cl_prt_cs3('csfr010','csfr010',l_sql,g_str)   #FUN-710080 modify


  CALL  cl_used(g_prog,g_time,2) RETURNING g_time #No.MOD-580088  HCN 20050818
END FUNCTION
#xueyh170303 add-----------start-------------
 FUNCTION csfr010_month(l_date)     ## 月份区间之落点
#-----------------------------------------------------------------------------#
...
#add by shijl 170321--str--
FUNCTION csfr010_tmp()
  DROP TABLE csfr010_tmp
  #add ima46 ima50 shijl 2017/3/29 10:04

             # imk02  LIKE imk_file.imk02,  mark by lixwz 20170728
  CREATE TEMP TABLE csfr010_tmp(
              sfa03  LIKE sfa_file.sfa03,
              ima02  LIKE ima_file.ima02,
              ima021 LIKE ima_file.ima021,
              ima08  LIKE ima_file.ima08,
