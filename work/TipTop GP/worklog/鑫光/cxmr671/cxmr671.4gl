...
# Modify.........: No.20170117  17/01/17 By cjy 如果报表类型选择三则不自动带出预设日期
# Modify.........: no.170408    17/04/08 by lifang 部分签收的出货单抓取不到
# Modify.........: add by lixwz 20170721  1、cxmr671查询窗口条件中添加“是否含销退”选项，勾选后将销退单也包含在对应逻辑中；
...
MAIN
   ...
   LET l_table = cl_prt_temptable('cxmr671',g_sql) CLIPPED   # 產生Temp Table
  ...
  #TQC-DC0088 mark --end--
   ...
   IF cl_null(g_bgjob) OR g_bgjob = 'N'# If background job sw is off
      THEN CALL cxmr671_tm(0,0)        # Input print condition
      ELSE CALL cxmr671()              # Read data and create out-file
   END IF
   CALL cl_used(g_prog,g_time,2) RETURNING g_time
END MAIN

FUNCTION cxmr671_tm(p_row,p_col)
...
DEFINE p_row,p_col    LIKE type_file.num5,
   ...
   OPEN WINDOW cxmr671_w AT p_row,p_col WITH FORM "cxm/42f/cxmr671"
...
WHILE TRUE
   INPUT BY NAME tm.rtype WITHOUT DEFAULTS
      ...
      BEFORE INPUT
         ...
         CALL r671_set_entry() #FUN-C30228
         CALL r671_set_no_entry() #FUN-C30228
   ...
   END INPUT
   IF INT_FLAG THEN
      LET INT_FLAG = 0 CLOSE WINDOW cxmr671_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF
  ...
  END DIALOG       #M044 161026 By TSD.Chin  add
   ...
   IF tm.rtype = '3'  THEN
      LET tm.edate = ''
   END IF
   #end add by cjy 20170117
   #LET tm.oha = 'N' # add by lixwz 20170721
   ...
   IF g_bgjob = 'Y' THEN
      ...
      ELSE
         ...
         CALL cl_cmdat('cxmr671',g_time,l_cmd)
      END IF
      CLOSE WINDOW cxmr671_w
      CALL cl_used(g_prog,g_time,2) RETURNING g_time
      EXIT PROGRAM
   END IF
   CALL cl_wait()
...
#FUN-C30228---add---START
FUNCTION r671_set_entry()
    CALL cl_set_comp_entry("edate",TRUE)
END FUNCTION

FUNCTION r671_set_no_entry()
   IF tm.rtype = '2' THEN
...
FUNCTION cxmr671()
 ...
 #no.170408 add begin#
   ...
   DEFINE l_sum_ogb912_8  LIKE ogb_file.ogb912
   DEFINE l_sum_ogb915_8  LIKE ogb_file.ogb915
   DEFINE l_sum_ogb917_8  LIKE ogb_file.ogb917
   ...
   #M044 161026 BY TSD.Chin ---add----------(E)
     ...
     IF tm.rtype = '1' THEN
        LET l_sql = "SELECT oga02,oga01,oga03,oga14,oga15,ogb04,ogb06,'',ogb12, ",
                   ...
                   # "  FROM oga_file,ogb_file,azi_file,occ_file ",  #0000685690_06_M024 160303 BY TSD.zhen Add  #mark by zl 170427
                    "  FROM oga_file,ogb_file,azi_file,occ_file,oea_file ",  #add by zl 170427
                   # " WHERE oga01 = ogb01 AND oga23 = azi01 AND oga65 ='Y' AND oga09 ='2' ",  #mark by zl 170427
                    " WHERE oga01 = ogb01 AND oga23 = azi01 AND ogb31 = oea01 AND oea49 = '1' AND oeaconf = 'Y' AND oga65 ='Y' AND oga09 ='2' ",  #add by zl 170427
...
#                    END IF
     ...
     IF tm.oha = '1' THEN
         LET tm.wc =cl_replace_str(tm.wc,"oga","oha")
         LET l_sql=l_sql ,
                    ...
                    " FROM oha_file,ohb_file,azi_file,occ_file",
                    "  WHERE   oha01=ohb01 and oha23=azi01 and oha03=occ01  AND ",tm.wc
     END IF
     # add by lixwz 20170721 e

     PREPARE cxmr671_prepare1 FROM l_sql
     IF SQLCA.sqlcode != 0 THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time
        EXIT PROGRAM
     END IF
     DECLARE cxmr671_curs1 CURSOR FOR cxmr671_prepare1
     ...
     LET l_sql = "SELECT tc_otn04,azf03 FROM tc_otn_file,azf_file ",
                 ...
                 " ORDER BY tc_otn03 "
     PREPARE r671_tc_otn_p FROM l_sql
     DECLARE r671_tc_otn_c CURSOR FOR r671_tc_otn_p
     #======0000685690_06_M024 160303 BY TSD.zhen======(E)
    ...
    LET l_sql2 = l_sql                #no.170408
     IF tm.rtype = '1' THEN
        ...
        END IF
#--no.170408 begin#
        LET l_sql2 = l_sql2 , " AND ogb66||ogb67 IN (SELECT ogb01||ogb03 FROM ogb_file,oga_file ",
                              "  WHERE oga01 = ogb01 AND ogb912-ogb50 <>0 AND oga09 in ('2','3' ) AND ogapost ='Y') "
                              #"  WHERE oga01 = ogb01 AND ogb917-ogb50 <>0 AND oga09 in ('2','3' ) AND ogapost ='Y') " #mod by cjy 20170508
        IF tm.wc2 <> ' 1=1' THEN
           LET l_sql2 = l_sql2,
                    " AND ", tm.wc2 CLIPPED
        END IF
...
#       LET l_sql = l_sql," UNION ",l_sql2
       ...
       PREPARE r671_n_p1_1 FROM l_sql2
       DECLARE r671_n_cur1_1 CURSOR FOR r671_n_p1_1
       ...
       LET l_sql3 = " SELECT COUNT(*) ",
        ...
        IF NOT cl_null(tm.edate) THEN
          LET l_sql3 = l_sql3 ,
                      " AND oga02 > '",tm.edate,"' "
        END IF
        PREPARE r671_n_p1_2 FROM l_sql3
#--no.170408 end#
     END IF
     IF tm.rtype = '3' THEN
        LET l_sql = l_sql,
     ...
     LET l_sql = l_sql,
                 " ORDER BY oga01"
     PREPARE r671_n_p1 FROM l_sql
     DECLARE r671_n_cur1 CURSOR FOR r671_n_p1
     ...
     LET l_sql = "SELECT ogb03 ",
                 ...
                 "   AND oga01 = ? ",
                 " ORDER BY ogb01 "
     PREPARE r671_n_p2 FROM l_sql
     DECLARE r671_n_cur2 CURSOR FOR r671_n_p2
     ...
     LET l_sql = "SELECT DISTINCT omb01",
                 ...
                 "   AND omavoid = 'N'"    #不可作廢
     PREPARE r671_p5 FROM l_sql
     DECLARE r671_cur5  CURSOR FOR r671_p5
     #M044 161028 By TSD.Tim---(E)

     #FOREACH cxmr671_curs1 INTO sr.*  #0000685690_06_M024 160303 BY TSD.zhen Mark
     FOREACH cxmr671_curs1 INTO sr.*,l_oga032,l_ogb14,l_ta_ogb01  #0000685690_06_M024 160303 BY TSD.zhen Add
                               #M029 160712 By TSD.nick ======(s)
      ...
      LET l_tc_otn04 = NULL
      LET l_desc = NULL
      FOREACH r671_tc_otn_c USING l_inb01 INTO l_tc_otn04,l_azf03
         IF STATUS THEN
            EXIT FOREACH
         END IF
      ...
      #            "   AND ", tm.wc2 CLIPPED,
      #            " ORDER BY oga01"
      #PREPARE r671_p1 FROM l_sql
      #DECLARE r671_n_cur1 CURSOR FOR r671_p1
      #LET l_sql = "SELECT DISTINCT omb01",
      #            "  FROM omb_file,oma_file",
      #            " WHERE omb01 = oma01",
      #            "   AND omb31 = ? ",
      ...
      LET sr.oga10 = NULL
   #--no.170408 begin#
      EXECUTE r671_n_p1_2 INTO l_count USING sr.ogb03 , sr.oga01
     IF cl_null(l_count) THEN LET l_count=0 END IF
     IF l_count > 0 THEN
   #--no.170408 end#
      FOREACH r671_n_cur1 USING sr.ogb03 , sr.oga01 INTO l_oga01a,l_oga02a
         LET l_flag = 'Y'
         IF SQLCA.sqlcode THEN
            CALL cl_err('foreach cxmr671_n_cur1:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF

        #M044 161028 By TSD.Tim---(S)
        FOREACH r671_n_cur2 USING sr.ogb03,sr.oga01,l_oga01a
                             INTO l_ogb03a
         IF SQLCA.sqlcode THEN
            CALL cl_err('foreach cxmr671_n_cur2:',SQLCA.sqlcode,1)
            EXIT FOREACH
         END IF
        #M044 161028 By TSD.Tim---(E)

         #M044 161028 By TSD.Tim---(S)
         #FOREACH r671_cur5 USING l_oga01a INTO l_omb01
         FOREACH r671_cur5 USING l_oga01a,l_ogb03a
                            INTO l_omb01
         #M044 161028 By TSD.Tim---(E)
            IF SQLCA.sqlcode THEN
               CALL cl_err('foreach cxmr671_cur5:',SQLCA.sqlcode,1)
               EXIT FOREACH
            END IF
      ...
      #--no.170408 begin#
       LET l_sum_ogb912_8 = 0
       LET l_sum_ogb915_8 = 0
       LET l_sum_ogb917_8 = 0
       SELECT SUM(ogb912),SUM(ogb915),SUM(ogb917)         #本张签收单签收量,参考数量,计价数量
       INTO l_sum_ogb912_8,l_sum_ogb915_8,l_sum_ogb917_8
         #FROM ogb_file
         FROM ogb_file,oga_file #mod by cjy 20170619
       WHERE ogb01 =  l_oga01a
        AND oga01 = ogb01 #mod by cjy 20170619
        AND ogb66 = sr.oga01
        AND ogb67 = sr.ogb03
        AND ogaconf = 'Y' #mod by cjy 20170619
        AND ogapost = 'Y' #mod by cjy 20170619
       SELECT oga211,oga23 INTO l_oga211,l_oga23
         FROM oga_file
       WHERE oga01 = sr.oga01
       SELECT azi03,azi04 INTO t_azi03,t_azi04
         FROM azi_file
       WHERE azi01=l_oga23
       IF cl_null(l_sum_ogb912_8) THEN LET l_sum_ogb912_8= 0 END IF
       IF cl_null(l_sum_ogb915_8) THEN LET l_sum_ogb915_8= 0 END IF
       IF cl_null(l_sum_ogb917_8) THEN LET l_sum_ogb917_8= 0 END IF
#       LET l_ogb912 = l_ogb912 - l_sum_ogb912_8
#       LET sr.ogb915 = sr.ogb915 - l_sum_ogb915_8
#       LET l_ogb917 = l_ogb917 - l_sum_ogb917_8
#       LET l_ogb14t = sr.ogb13 * l_ogb912
#       LET l_ogb14  = l_ogb14t/(1+(l_oga211/100))
        LET l_ogb912 = l_sum_ogb912_8
        LET sr.ogb915 = l_sum_ogb915_8
        LET l_ogb917 = l_sum_ogb917_8
        LET l_ogb14t = sr.ogb13 * l_ogb912
        LET l_ogb14  = l_ogb14t/(1+(l_oga211/100))
       CALL cl_digcut(l_ogb14,t_azi04) RETURNING l_ogb14
       LET l_str2 = ""
      IF g_sma.sma115 = "Y" THEN
         CASE l_ima906
            WHEN "2"
      ...
      EXECUTE insert_prep USING sr.oga02 ,sr.oga01 ,sr.oga03,sr.oga14,sr.oga15,
                               ...
                               ,l_oga032 ,l_ogb14  ,l_ta_ogb01,l_desc  #0000685690_06_M024 160303 BY TSD.zhen Add
                               #M029 160712 By TSD.nick ====(s)
...
#--no.170408 begin#
    ELSE
      FOREACH r671_n_cur1_1 USING sr.ogb03 , sr.oga01 INTO l_oga01a,l_oga02a
         LET l_flag = 'Y'
         IF SQLCA.sqlcode THEN
            CALL cl_err('foreach cxmr671_n_cur1:',SQLCA.sqlcode,1)
            EXIT FOREACH
     ...
     #--no.170408 begin#
       ...
       INTO l_sum_ogb912_8,l_sum_ogb915_8,l_sum_ogb917_8
         ...
         FROM ogb_file,oga_file #mod by cjy 20170619
       WHERE ogb01 =  l_oga01a
        AND oga01= ogb01  #add by cjy 20170619
        AND ogb66 = sr.oga01
        AND ogb67 = sr.ogb03
        AND ogaconf = 'Y' #mod by cjy 20170619
        AND ogapost = 'Y'  #add by cjy 20170619
       SELECT oga211,oga23 INTO l_oga211,l_oga23
         FROM oga_file
       WHERE oga01 = sr.oga01
       SELECT azi03,azi04 INTO t_azi03,t_azi04
       ...
       LET l_ogb917 = l_ogb917 - l_sum_ogb917_8
       #LET l_ogb14t = sr.ogb13 * l_ogb912
       LET l_ogb14t = sr.ogb13 * l_ogb917   #mod by cjy 20170701
       LET l_ogb14  = l_ogb14t/(1+(l_oga211/100))
       CALL cl_digcut(l_ogb14,t_azi04)  RETURNING l_ogb14
       #LET sr.ogb14t = l_ogb14t - l_ogb14  #add by cjy 20170701
       LET l_str2 = ""
      IF g_sma.sma115 = "Y" THEN
         CASE l_ima906
            WHEN "2"
    ...
    END IF
#--no.170408 end#
			# add by lixwz 20170705 srart
			FREE r671_n_cur1_1
			# add by lixwz 20170705 end
      #M044 161028 By TSD.Tim---(S)
      ...
      IF l_flag = 'N' THEN
         EXECUTE insert_prep USING sr.oga02 ,sr.oga01 ,sr.oga03,sr.oga14,sr.oga15,
                          ...
                          l_ogb13  ,l_ogb14t ,
                          sr.azi03 ,sr.azi04 ,sr.azi05
                         ,l_oga032 ,l_ogb14  ,l_ta_ogb01,l_desc
                         ,l_oea02,l_azf03a,l_ta_ogb02,l_oeb910,l_oeb912
                         ,l_ogb910,l_ogb912,l_ogb916,l_ogb917
                         #M044 161028 By TSD.Tim---(S)
                         #,sr.ogb03,l_gen02,sr.ogb31,l_ta_ima01,l_str2,sr.oga23,l_oga02a,l_oga01a,sr.oga10
     ...
     SELECT zz05 INTO g_zz05 FROM zz_file WHERE zz01 = 'cxmr671'
     IF g_zz05 = 'Y' THEN
     ...
     PREPARE cxmr671_preparet FROM l_sql
     IF SQLCA.sqlcode THEN
        CALL cl_err('prepare:',SQLCA.sqlcode,1)
        CALL cl_used(g_prog,g_time,2) RETURNING g_time
        EXIT PROGRAM
     ...
     CALL cl_prt_cs3('cxmr671','cxmr671',l_sql,l_str)
END FUNCTION
#FUN-BC0022
