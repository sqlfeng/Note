...
# Modify.........: No:MOD-DC0174 13/12/26 By yinhy FUNCTION t400_omc 中,oma00非23時,oob19條件改為(oob19 = b_oob.oob19 OR oob19 IS NULL)
# Modify.........: No:yinhy140418 14/04/18 By yinhy 应收账款参数设置收款冲账需冲至项次，有订金收款冲账時，貸方產生的應收金額錯誤
# Modify.........: No:MOD-E70117 14/07/22 By qiull 修改付款日期之後,tic現金流量明細重複
...
DEFINE  lc_qbe_sn       LIKE    gbm_file.gbm01
   ...
   INITIALIZE g_ooa.* TO NULL
      ...
      CONSTRUCT BY NAME g_wc ON ooa01,ooa02,ooa03,
                               #ooa35,ooa36,  #mark by renkl160126
                                ooa032,ooa14,ooa15,ooa23,
                               #ooa38,#TQC-C20430 add ooa35,ooa36 #FUN-C30029 add ooa38  #mark by rekl160126
...
FUNCTION t400_menu()
   ...
   DEFINE l_zz  LIKE type_file.num5   #add by renkl160127
   DEFINE l_cmd LIKE type_file.chr1000  #add by renkl160127
   LET l_zz = 0    #add by renkl160127
   LET l_flowuser = "N"
...
#FUN-A40076--Add--Begin
         WHEN "adjust_charge"
            ...
            END IF
#FUN-A40076--Add--End
#add by renkl160127 start
         WHEN "anmt200"
           IF cl_chk_act_auth() THEN
             LET l_zz = ARR_CURR()
              IF g_oob[l_zz].oob03='1' AND g_oob[l_zz].oob04='1' THEN
                LET l_cmd = "anmt200 '",g_oob[l_zz].oob06,"'"
         ...
         WHEN "anmt302"
           IF cl_chk_act_auth() THEN
             LET l_zz = ARR_CURR()
              IF g_oob[l_zz].oob03='1' AND g_oob[l_zz].oob04='2' THEN
                LET l_cmd = "anmt302 '",g_oob[l_zz].oob06,"'"
                CALL cl_cmdrun_wait(l_cmd)
              END IF
           END IF
         WHEN "axrt300a"
           IF cl_chk_act_auth() THEN
             LET l_zz = ARR_CURR()
              IF g_oob[l_zz].oob03='1' AND g_oob[l_zz].oob04='3' THEN
                LET l_cmd = "axrt300 '",g_oob[l_zz].oob06,"'"
                CALL cl_cmdrun_wait(l_cmd)
              END IF
           END IF
         WHEN "axrt300b"
           IF cl_chk_act_auth() THEN
             LET l_zz = ARR_CURR()
                IF g_oob_d[l_zz].oob03_d='2' AND g_oob_d[l_zz].oob04_1='1' THEN
                    LET l_cmd = "axrt300 '",g_oob_d[l_zz].oob06_1,"'"
                    CALL cl_cmdrun_wait(l_cmd)
                END IF
           END IF
#add by renkl160127 end
     END CASE
   END WHILE
END FUNCTION
...
FUNCTION t400_g_b2()
   ...
   DEFINE l_oea61        LIKE oea_file.oea61    #No:yinhy140418
   DEFINE l_oea1008      LIKE oea_file.oea1008  #No:yinhy140418
   DEFINE l_per          LIKE oea_file.oea261   #No:yinhy140418
      ...
      FOREACH omab_tmp_c INTO s_omab[i].*,l_oma00,l_omb34,l_omb14t
       ...
       ###當資料>最大筆數時，將後面的資料刪除(因為畫面出不來)
        ...
        IF g_ooz.ooz62='Y' THEN
           ...
           #No.yinhy140418  --Begin
           LET g_sql="SELECT * FROM oma_file  WHERE oma01=?"
           PREPARE t400_oob06_item_p2 FROM g_sql
           DECLARE t400_oob06_item_c2 CURSOR FOR t400_oob06_item_p2
           OPEN t400_oob06_item_c2 USING s_omab[i].omb01
           FETCH t400_oob06_item_c2 INTO l_oma.*
           ...
           IF l_oma.oma00='11' OR l_oma.oma00='12' OR l_oma.oma00='13' THEN
              ...
              IF l_oma.oma213 = 'Y' THEN
                 LET s_omab[i].omb14t = l_omb.omb14t * l_per / l_oea1008
                 LET s_omab[i].omb16t = l_omb.omb16t * l_per / l_oea1008
              ELSE
                 LET s_omab[i].omb14t = l_omb.omb14t * l_per / l_oea61
                 LET s_omab[i].omb16t = l_omb.omb16t * l_per / l_oea61
              END IF
           ELSE
              LET s_omab[i].omb14t = l_omb.omb14t
              LET s_omab[i].omb16t = l_omb.omb16t
           END IF
           #No.yinhy140418  --End
        ELSE
           SELECT SUM(oob09),SUM(oob10) INTO un_conf_amt1, un_conf_amt2
     ...
     FOREACH t400_g_b_c212 INTO l_oma.*,l_omb.*,l_omc02 #MOD-970255 add omc
      ...
      #------------- 970123 roger 檢查其他未確認沖帳資料, 以免重複
       ...
       IF l_oma.oma00='11' OR l_oma.oma00='12' OR l_oma.oma00='13' THEN
          ...
          END IF
          IF l_oma.oma213 = 'Y' THEN
             LET l_omb.omb14t = l_omb.omb14t * l_per / l_oea1008
             LET l_omb.omb16t = l_omb.omb16t * l_per / l_oea1008
          ELSE
             LET l_omb.omb14t = l_omb.omb14t * l_per / l_oea61
             LET l_omb.omb16t = l_omb.omb16t * l_per / l_oea61
          END IF
       END IF
       #No.yinhy140418  --End
       IF un_conf_amt1 IS NULL THEN LET un_conf_amt1 = 0 END IF
...
FUNCTION t400_u()
   ...
   DEFINE l_aza17     LIKE aza_file.aza17        # add by lixwz 20171016
       ...
       INPUT BY NAME g_ooa.ooa02,g_ooa.ooa03,g_ooa.ooa13,g_ooa.ooa14,   #MOD-9A0204 add ooa03
           ...
           AFTER FIELD ooa02
             ...
             IF g_ooz.ooz07 = 'Y' THEN
                ...
                # 外币时才判断 录入的日期小於月底重评价年度月份, 请检查!
                SELECT UNIQUE aza17 INTO l_aza17 FROM aza_file
                IF NOT cl_null(g_ooa.ooa23) AND g_ooa.ooa23 != l_aza17 THEN
           ...
           AFTER FIELD ooa03
              IF NOT cl_null(g_ooa.ooa03) THEN
                 ...
                 IF STATUS THEN
                    ...
                    NEXT FIELD ooa03
                 END IF
         DISPLAY l_occ.occ18 TO FORMONLY.occ18    #add by renkl160126
                 IF l_occ.occacti = 'N' THEN
                    CALL cl_err(l_occ.occ01,'9028',0) NEXT FIELD ooa03
                 END IF
                #LET g_ooa.ooa23 = l_occ.occ42 DISPLAY BY NAME g_ooa.ooa23   #mark by renkl160127
         ...
         AFTER FIELD ooa23
                ...
                IF g_ooz.ooz07 = 'Y' THEN
                   ...
                   IF NOT cl_null(g_ooa.ooa23) AND g_ooa.ooa23 != l_aza17 THEN
                       IF (l_yy*12+l_mm) - (g_ooz.ooz05*12+g_ooz.ooz06) < 1 THEN  #MOD-8B0213
                          CALL cl_err(g_ooa.ooa02,'axr-405',0)
                          NEXT FIELD ooa023   #MOD-830047
                       END IF
                       IF (l_yy*12+l_mm) - (g_ooz.ooz05*12+g_ooz.ooz06) = 0 THEN
                          CALL cl_err(g_ooa.ooa02,'axr-406',0) NEXT FIELD ooa023
                       END IF
                   END IF # add by lixwz 20171016 e
                END IF
                # add by lixwz 171016 e
...
DEFINE l_tic02     LIKE tic_file.tic02   #MOD-E70117 add
   IF cl_null(g_ooa.ooa33) THEN
      ...
      IF g_nmz.nmz70 = '3' THEN
         LET l_tic01=YEAR(g_ooa.ooa02)
         LET l_tic02=MONTH(g_ooa.ooa02)
...
FUNCTION t400_i(p_cmd)
  ...
  DEFINE l_aza17         LIKE aza_file.aza17               # add by lixwz 171016

    CALL cl_set_head_visible("","YES")       #No.FUN-6A0092

    INPUT BY NAME
      # g_ooa.ooaoriu,g_ooa.ooaorig,   #mark by renkl160126
        g_ooa.ooa01,g_ooa.ooa02,g_ooa.ooa03,g_ooa.ooa032,
        g_ooa.ooa14,g_ooa.ooa15,g_ooa.ooa23,
       #g_ooa.ooa38,  #mark by renkl160126
        g_ooa.ooa13,g_ooa.ooa40,  #FUN-C20018 add ooa40 #FUN-C30029 add ooa38
        ...
        g_ooa.ooaoriu,g_ooa.ooaorig,    #add by renkl160126
...
#No.FUN-AA0088 --end
       ...
       DISPLAY l_occ.occ18 TO FORMONLY.occ18    #add by renkl160126
             IF l_occ.occacti = 'N' THEN
                CALL cl_err(l_occ.occ01,'9028',0) NEXT FIELD ooa03
             END IF
          # IF g_ooa.ooa23 IS NULL THEN   #mark by renkl160127 start
          #     LET g_ooa.ooa23 = l_occ.occ42 DISPLAY BY NAME g_ooa.ooa23
          #  END IF                      #mark by renkl160127 end
             IF g_ooa.ooa03 != 'MISC' THEN
                LET g_ooa.ooa032 = l_occ.occ02 DISPLAY BY NAME g_ooa.ooa032
             END IF
          END IF
...
FUNCTION t400_show()
DEFINE l_occ18  LIKE occ_file.occ18    #add by renkl160126
    LET g_ooa_t.* = g_ooa.*                #保存單頭舊值

    DISPLAY BY NAME g_ooa.ooa01,g_ooa.ooa02,g_ooa.ooa03,
                   #g_ooa.ooa35,g_ooa.ooa36,   #mark by renkl160127
                    g_ooa.ooa032, g_ooa.ooaoriu,g_ooa.ooaorig, #TQC-C20430 add ooa35,ooa36
                    g_ooa.ooa14,g_ooa.ooa15,g_ooa.ooa23,
                   #g_ooa.ooa38, #mark by renkl160127
                    g_ooa.ooa13,g_ooa.ooa40, #FUN-C20018 add ooa40  #FUN-C30029 add ooa38
    ...
    IF g_ooa.ooaconf='X' THEN LET g_chr='Y' ELSE LET g_chr='N' END IF
    IF g_ooa.ooa34 = '1' THEN LET g_chr2='Y' ELSE LET g_chr2='N' END IF
 #add by renkl160126 start
    SELECT occ18 INTO l_occ18 FROM occ_file WHERE occ01 = g_ooa.ooa03
      DISPLAY l_occ18 TO FORMONLY.occ18
#add by renkl16126 end
    CALL cl_set_field_pic(g_ooa.ooaconf,g_chr2,"","",g_chr,"")
    CALL t400_show2()
    CALL t400_show_amt()
...
## No.2693 modify 1998/10/31 至少需判斷幣別為正確值
           IF NOT cl_null(g_oob[l_ac].oob07) THEN
              ...
              IF g_ooz.ooz07 = 'Y' THEN
                 ...
                 # 外币时才判断 录入的日期小於月底重评价年度月份, 请检查!
                 SELECT UNIQUE aza17 INTO l_aza17 FROM aza_file
                 IF NOT cl_null(g_oob[l_ac].oob07) AND g_oob[l_ac].oob07 != l_aza17 THEN
                     IF (l_yy*12+l_mm) - (g_ooz.ooz05*12+g_ooz.ooz06) < 1 THEN  #MOD-8B0213
                        CALL cl_err(g_ooa.ooa02,'axr-405',0)
                        NEXT FIELD oob07   #MOD-830047
                     END IF
                     IF (l_yy*12+l_mm) - (g_ooz.ooz05*12+g_ooz.ooz06) = 0 THEN
                        CALL cl_err(g_ooa.ooa02,'axr-406',0) NEXT FIELD oob07
                     END IF
                 END IF # add by lixwz 20171016 e
              END IF
              # add by lixwz 171016 e
...
#MOD-590452 將原本mark的部份取消
        ...
        ON ACTION receive_notes
           IF g_oob[l_ac].oob03='1' AND g_oob[l_ac].oob04='1' THEN
              LET l_cmd = "anmt200 ",  #add by renkl160127
                          "'",g_oob[l_ac].oob06,"'"   #add by renkl160127
              CALL cl_cmdrun_wait(l_cmd)      #add by renkl160127
            # CALL cl_cmdrun_wait('anmt200')  #FUN-660216 add #mark by renkl160127
           END IF

        ON ACTION bank_income_expense
           IF g_oob[l_ac].oob03='1' AND g_oob[l_ac].oob04='2' THEN
              LET l_cmd = "anmt302 ",  #add by renkl160127
                          "'",g_oob[l_ac].oob06,"'"   #add by renkl160127
              CALL cl_cmdrun_wait(l_cmd)      #add by renkl160127
             #CALL cl_cmdrun_wait('anmt302')  #FUN-660216 add   #mark by renkl160127
           END IF

        ON ACTION maintain_accounts
           IF g_oob[l_ac].oob03='1' AND g_oob[l_ac].oob04='3' THEN
              LET l_cmd = "axrt300 ",  #add by renkl160127
                          "'",g_oob[l_ac].oob06,"'"   #add by renkl160127
              CALL cl_cmdrun_wait(l_cmd)      #add by renkl160127
            # CALL cl_cmdrun_wait('axrt300')  #FUN-660216 add   #mark by renkl160127
           END IF
           IF g_oob[l_ac].oob03='2' AND g_oob[l_ac].oob04='1' THEN
...
#add by renkl160127 start
     ON ACTION anmt200
        LET g_action_choice="anmt200"
        EXIT DIALOG
     ON ACTION anmt302
        LET g_action_choice="anmt302"
        EXIT DIALOG
     ON ACTION axrt300a
        LET g_action_choice="axrt300a"
        EXIT DIALOG
     ON ACTION axrt300b
        LET g_action_choice="axrt300b"
        EXIT DIALOG
#add by renkl160127 end
      ON ACTION void  #作廢
...
FUNCTION t400_out()
    ...
    MENU ""
       ...
       ON ACTION print_this_pay_slip
         LET g_msg = "axrr400 '' '' '",g_lang,"' 'Y' '' '' ",  #FUN-C30085   #radd by pulf 141225
                     "'ooa01 =\"",g_ooa.ooa01,"\"' 'g_ooa.ooaconf' "     #add by pulf 141225
      #mark by pulf 141225---begin-----------
          #No.FUN-BB0100  --Begin
        #  IF g_artype ='33' THEN
        #     #FUN-D10098--add--str--
...
FUNCTION t400_y_chk()                # when g_ooa.ooaconf='N' (Turn to 'Y')
   ...
   DEFINE l_oob      RECORD
             ...
             oob06    LIKE oob_file.oob06
                     END RECORD
   # add by lixwz 171017 s
   DEFINE l_i 			  LIKE type_file.num5
   DEFINE l_aza17     LIKE aza_file.aza17
   DEFINE l_yy,l_mm   LIKE type_file.num5
   # add by lixwz 171017 e

   LET g_success = 'Y'
   # add by lixwz 171017 s
   FOR l_i = 1 TO g_oob.getLength()
   		IF g_ooz.ooz07 = 'Y' THEN
   		   LET l_yy = YEAR(g_ooa.ooa02)
   		   LET l_mm = MONTH(g_ooa.ooa02)
   		   # 外币时才判断 录入的日期小於月底重评价年度月份, 请检查!
   		   SELECT UNIQUE aza17 INTO l_aza17 FROM aza_file
   		   IF NOT cl_null(g_oob[l_i].oob07) AND g_oob[l_i].oob07 != l_aza17 THEN
   		       IF (l_yy*12+l_mm) - (g_ooz.ooz05*12+g_ooz.ooz06) < 1 THEN  #MOD-8B0213
   		          CALL cl_err(g_ooa.ooa02,'axr-405',1)
   		          LET g_success = 'N'
   		          RETURN
   		       END IF
   		       IF (l_yy*12+l_mm) - (g_ooz.ooz05*12+g_ooz.ooz06) = 0 THEN
   		          CALL cl_err(g_ooa.ooa02,'axr-406',1) #NEXT FIELD oob07
   		          LET g_success = 'N'
   		          RETURN
   		       END IF
   		   END IF
   ...
   END FOR
   # add by lixwz 171017 e
   FOR l_i = 1 TO g_oob_d.getLength()
   		IF g_ooz.ooz07 = 'Y' THEN
   		   LET l_yy = YEAR(g_ooa.ooa02)
   		   LET l_mm = MONTH(g_ooa.ooa02)
   		   # 外币时才判断 录入的日期小於月底重评价年度月份, 请检查!
   		   SELECT UNIQUE aza17 INTO l_aza17 FROM aza_file
   		   IF NOT cl_null(g_oob_d[l_i].oob07_1) AND g_oob_d[l_i].oob07_1 != l_aza17 THEN
   		       IF (l_yy*12+l_mm) - (g_ooz.ooz05*12+g_ooz.ooz06) < 1 THEN  #MOD-8B0213
   		          CALL cl_err(g_ooa.ooa02,'axr-405',1)
   		          LET g_success = 'N'
   		          RETURN
...
#--------------------------------No.MOD-B80122-----------------------------end
      SELECT SUM(oob09),SUM(oob10) INTO l_oob09,l_oob10 FROM oob_file, ooa_file
      #WHERE oob06=b_oob.oob06 AND oob19 = b_oob.oob19                     #MOD-DC0174 mark
       WHERE oob06=b_oob.oob06 AND (oob19 = b_oob.oob19 OR oob19 IS NULL)  #MOD-DC0174
         AND oob01=ooa01  AND ooaconf = 'Y'
...
FUNCTION t400_oob06_1_item_2()      # 檢查待抵/應收帳款
  ...
  #-MOD-B70102-end-
   ...
   IF l_oma.oma00='11' OR l_oma.oma00='12' OR l_oma.oma00='13' THEN
      ...
      #IF g_oma.oma213 = 'Y' THEN      #yinhy140418
      IF l_oma.oma213 = 'Y' THEN       #yinhy140418
         LET l_omb14t = l_omb.omb14t * l_per / l_oea1008
         LET l_omb16t = l_omb.omb16t * l_per / l_oea1008
      ELSE
         LET l_omb14t = l_omb.omb14t * l_per / l_oea61
...
FUNCTION t400_set_comb_oob04()
  ...
  DEFINE comb_item  LIKE type_file.chr1000
   ...
   #LET comb_value = '1,2,3,4,5,6,7,8,9,E,F,H,Q'        #FUN-D70029 add 'H' #mark by renkl160127
   # LET comb_value = '1,2,3,5,6,7,9'     #add by renkl160127   #mark by kuangxj170830
     LET comb_value = '1,2,3,4,5,6,7,9'   #add by kuangxj170830
    SELECT ze03 INTO comb_item FROM ze_file
#     WHERE ze01='axr-218' AND ze02=g_lang
     WHERE ze01='cxr-218' AND ze02=g_lang   #add by kuangxj170830
...
#No.FUN-AA0088 -begin
   ...
   #LET comb_value = '1,2,4,7,9,B'   #MOD-B50156  #mark by renkl160127
    LET comb_value = '1,2,7,9'             #add by renkl160127
...
#No.FUN-AA0088 --end
        ...
        ON ACTION bank_income_expense
                 IF g_oob_d[l_ac2].oob03_d='1' AND g_oob_d[l_ac2].oob04_1='2' THEN
                    CALL cl_cmdrun_wait('anmt302')
                 END IF
      #ON ACTION maintain_accounts   #mark by renkl160127
       ON ACTION ysd    #add by renkl160127
                 IF g_oob_d[l_ac2].oob03_d='1' AND g_oob_d[l_ac2].oob04_1='3' THEN
                    CALL cl_cmdrun_wait('axrt300')
                 END IF
                 IF g_oob_d[l_ac2].oob03_d='2' AND g_oob_d[l_ac2].oob04_1='1' THEN
                    LET l_cmd = "axrt300 ",  #add by renkl160127
                                "'",g_oob_d[l_ac2].oob06_1,"'"   #add by renkl160127
                    CALL cl_cmdrun_wait(l_cmd)      #add by renkl160127
                   #CALL cl_cmdrun_wait('axrt300')   #mark by renkl160127
                 END IF
        ON ACTION ar_account_category
                    CALL cl_cmdrun('axri040')
