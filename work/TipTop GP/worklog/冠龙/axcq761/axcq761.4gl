# Prog. Version..: '5.30.24-17.06.30(00010)'     #
#
...
# Modify.........: No.MOD-E50023 14/05/06 By Alberti tlf920的條件，增加是否為大陸版的判斷，台灣不用tlf920欄位
# Modify.........: No.MOD-E50115 14/05/27 By Alberti 1.在銷貨收入(wsaleamt)要*-1時，多判斷oma00(帳款類別)不為1%
#                                                    2.多判斷oma76=' '的狀況，因oma76有可能為空值
# Modify.........: No.MOD-E90099 14/09/23 By Alberti 主SQL中串oma_file應使用left outer join，避免當出貨/銷退尚未立應收時，查詢不到該筆出貨/銷退項次資料
# Modify.........: No.MOD-F20003 15/02/02 By Mandy 寫入ckk_file段落FROM axcq761筆誤寫錯
# Modify.........: No.MOD-F20025 15/02/05 By Mandy EXECUTE q761_wsaleamt03 MERGE 指令會產生-6372(ORA-30926) 
# Modify.........: No.MOD-F30100 15/03/18 By Alberti 增加 6:移動加權平均
# Modify.........: No.MOD-G20060 16/02/22 By Mandy 查詢非關係人時,資料異常,卻全都是關係人
# Modify.........: No.MOD-G30151 16/03/25 By Mandy 多倉出貨時,明細資料內的銷售收入,資料異常"全都算到某一筆,其餘為0"
# Modify.........: No.MOD-G50005 16/05/03 By catmoon INSERT INTO axcq761_tmp 的SQL,需排除應收已作廢(omavoid='Y')的單據
# Modify.........: No.MOD-G90016 16/09/03 By Mandy MOD-G30151的調整,導致當此筆應收稅別是含稅時,單價也是含稅,會變成含稅金額 ,但銷貨收入應抓未稅金額
# Modify.........: No.MOD-H60012 17/06/15 By Mandy SQL 內因為有寫到NVL(omb14/omb12,0) 當omb12 為0 時,會有-6372異常(ORA-01476)
# Modify.........: No.MOD-H60025 17/06/30 By Mandy 程式(omb14/omb12) 抓出來的為(原幣未稅金額/數量) 沒有換算成本幣,故需再*oma24(匯率)
   ...
   DIALOG ATTRIBUTE(UNBUFFERED)
      INPUT BY NAME tm.bdate,tm.edate,tm.type,         
         ...
         ON CHANGE edate
            ...
            END IF                           
         AFTER FIELD type                                              
           #IF tm.type NOT MATCHES '[12345]' THEN NEXT FIELD type END IF  #MOD-F30100 mark
            IF tm.type NOT MATCHES '[123456]' THEN NEXT FIELD type END IF #MOD-F30100 add
...
#FUN-D10022----------------add-------------end-----------------------
      ...
      LET l_sql = "SELECT tlf905,tlf906,tlf01,ima02,ima021,tlf19,occ02,NVL(SUM(tlf10*tlf60*tlf907),0),NVL(SUM(tlfc21*tlf907),0),",
                 ...
                 #"       NVL(SUM(tlfc224*tlf907),0),NVL(SUM(tlfc2241*tlf907),0),NVL(SUM(tlfc2242*tlf907),0),NVL(SUM(tlfc2243*tlf907),0),tlf13,tlf66         ,NVL(azf08,'N'),' ' ogb01,0 ogb03 ",      #FUN-CC0157 add ogb01,ogb03 #MOD-G90016 mark
                  "       NVL(SUM(tlfc224*tlf907),0),NVL(SUM(tlfc2241*tlf907),0),NVL(SUM(tlfc2242*tlf907),0),NVL(SUM(tlfc2243*tlf907),0),tlf13,NVL(tlf66,'Y'),NVL(azf08,'N'),' ' ogb01,0 ogb03 ",      #FUN-CC0157 add ogb01,ogb03 #MOD-G90016 add
                  "       ,tlf907,oma00", #MOD-D90159 add #MOD-E50115 add oma00
                  "  FROM ",cl_get_target_table(m_plant[l_i],'ima_file'),",",cl_get_target_table(m_plant[l_i],'tlf_file'),
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'occ_file')," ON tlf19=occ01 ",
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'omb_file')," ON tlf905 = omb31 AND tlf906 = omb32 ", #MOD-E50115 add
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'oma_file')," ON oma01 = omb01 ",                     #MOD-E90099 add
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'azf_file')," ON tlf14=azf01 AND azf02='2' ",   
                  "  LEFT OUTER JOIN ",cl_get_target_table(m_plant[l_i],'tlfc_file')," ON tlfc01 = tlf01 ",
                  "   AND tlfc06 = tlf06  AND tlfc02 = tlf02  AND tlfc03 = tlf03 AND ",
                  "  tlfc13 = tlf13  AND tlfc902= tlf902 AND tlfc903= tlf903 AND ",
                 ...
                 #"  ,",cl_get_target_table(m_plant[l_i],'oma_file'),      #MOD-E50115 add  #MOD-E90099 mark
                  " WHERE ima01 = tlf01", 
                  "   AND (tlf13 LIKE 'axmt%' OR tlf13 LIKE 'aomt%')",
                  "   AND omavoid <> 'Y' ",    #MOD-G50005 add
                  "   AND ",tm.wc CLIPPED,
                  "   AND ",g_filter_wc CLIPPED,
                 #"   AND oma01 = omb01",                                  #MOD-E50115 add   #MOD-E90099 mark
                  "   AND tlf902 not in (SELECT jce02 from ",cl_get_target_table(m_plant[l_i],'jce_file'),")" 
      #MOD-E50023-Start-Modify
      IF g_aza.aza26 = '2' THEN    #當區域別為大陸，多加tlf920判斷          
         LET l_sql = l_sql CLIPPED, "   AND tlf920='Y' "
      END IF           
      LET l_sql = l_sql CLIPPED, 
      #MOD-E50023-End-Modify 
                 #"   AND tlf920='Y' ",   #MOD-E50023 mark 
                  "   AND (tlf06 BETWEEN '",tm.bdate,"' AND '",tm.edate,"')",
                  "   AND tlf905 LIKE TRIM(smyslip)||'-%' AND smydmy1 ='Y' ",    #No.MOD-D30160
                 #" GROUP BY tlf01,ima02,ima021,ima131,occ02,occ03,occ04,occ37,tlf036,tlf13,tlf19,tlf66,tlf905,tlf906,azf08,tlf907,oma00 ", #MOD-D90159 add tlf907 #MOD-E50115 add oma00 #MOD-G30151 mark
                 #" GROUP BY tlf01,ima02,ima021,ima131,occ02,occ03,occ04,NVL(occ37,'N'),tlf036,tlf13,tlf19,tlf66,         tlf905,tlf906,NVL(azf08,'N'),tlf907,oma00 ",                            #MOD-G30151 add 要寫NVL(occ37,'N') 不能寫occ37,否則實際上未將occ37 相同的資料GROUP BY 成同一筆  #MOD-G90016 mark
                  " GROUP BY tlf01,ima02,ima021,ima131,occ02,occ03,occ04,NVL(occ37,'N'),tlf036,tlf13,tlf19,NVL(tlf66,'Y'),tlf905,tlf906,NVL(azf08,'N'),tlf907,oma00 ",                            #MOD-G30151 add 要寫NVL(occ37,'N') 不能寫occ37,否則實際上未將occ37 相同的資料GROUP BY 成同一筆  #MOD-G90016 add
      ...
      LET l_sql = "MERGE INTO axcq761_tmp o",
                 ...
                 #"  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 AND (o.tlf905 = x.oma76 OR x.oma76 IS NULL))", #MOD-D90121 add   #MOD-E50115 mark
                  "  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 AND (o.tlf905 = x.oma76 OR x.oma76 IS NULL OR x.oma76 = ' '))",  #MOD-E50115 add
                  " WHEN MATCHED ",
                  " THEN ",
                  "   UPDATE ",
#                  "      SET o.wsaleamt = o.wsaleamt+ x.omb16 *(-1) ",
      ...
      LET l_sql = "MERGE INTO axcq761_tmp o",
                 #"  USING( SELECT DISTINCT oma00,NVL(omb16,0) omb16,omb38,omb31,omb32 ",         #MOD-D90121 mark
                 #"  USING( SELECT DISTINCT oma00,NVL(omb16,0) omb16,omb38,omb31,omb32,oma76 ",   #MOD-D90121 add #MOD-G30151 mark
                 #"  USING( SELECT DISTINCT oma00,NVL(omb15,0) omb15      ,omb38,omb31,omb32,oma76 ",   #MOD-D90121 add #MOD-G30151 add  #MOD-G90016 mark
                 #MOD-H60012---mod---str----
                 #"  USING( SELECT DISTINCT oma00,NVL(omb14/omb12,0) omb15,omb38,omb31,omb32,oma76 ",   #MOD-D90121 add #MOD-G30151 add  #MOD-G90016 add
                 #"  USING( SELECT DISTINCT oma00,case when omb12=0 then omb15 else NVL(omb14/omb12,0) end omb15, ",       #MOD-H60025 mark
                  "  USING( SELECT DISTINCT oma00,case when omb12=0 then omb15 else NVL(omb14/omb12,0)*oma24 end omb15, ", #MOD-H60025 add
                  "                                                        omb38,omb31,omb32,oma76 ",   
                 #MOD-H60012---mod---end----
                  "           FROM ",cl_get_target_table(m_plant[l_i],'oma_file'),",", 
                                     cl_get_target_table(m_plant[l_i],'omb_file'),
                  "          WHERE oma01=omb01 AND (oma02 BETWEEN '",tm.bdate,"' AND '",tm.edate,"')",
                 ...
                 #"  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 )",  #FUN-CC0157 mod                           #MOD-D90121 mark
                 #"  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 AND (o.tlf905 = x.oma76 OR x.oma76 IS NULL))", #MOD-D90121 add   #MOD-E50115 mark
                 #"  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 AND (o.tlf905 = x.oma76 OR x.oma76 IS NULL OR x.oma76 = ' '))",  #MOD-E50115 add                  #MOD-F20025 mark
                  "  ON (o.ogb01 =x.omb31 AND o.ogb03 =x.omb32 AND o.oma00 =x.oma00 AND (o.tlf905 = x.oma76 OR x.oma76 IS NULL OR x.oma76 = ' '))",  #MOD-E50115 #MOD-F20025 add  oma00,因為USING TABLE x內,ON..條件下不可有重覆值,否則會有ORA-30926 異常
                  " WHEN MATCHED ",
                  " THEN ",
                  "   UPDATE ",
                 #"      SET o.wsaleamt = o.wsaleamt+ x.omb16 ",              #MOD-G30151 mark 
                  "      SET o.wsaleamt = o.wsaleamt+ o.tlf10*(-1)*x.omb15 ", #MOD-G30151 add  因為多倉出貨時,二筆tlf_file都對到同一筆金額omb16,導致金額會重覆,故改用數量*單價
                  "                       -(SELECT NVL(SUM(oct12),0) FROM oct_file ",
                  "                             WHERE oct04 = o.tlf905 AND oct05 = o.tlf906 ",
                  "                                   AND oct16='1' AND oct00='0') ",
                  "                       + (SELECT NVL(SUM(oct14),0) FROM oct_file ",
                  ...
                  "                               AND (oct09*12)+oct10 BETWEEN ",l_bdate," AND ",l_edate,
                  "                               AND oct00 = '0')",
                 #"    WHERE (o.rowno=1 OR o.tlf66 = 'X') AND o.tlf10 <=0 " #MOD-G30151 mark
                  "    WHERE o.tlf10 <=0 "                                  #MOD-G30151 add  因為多倉出貨時,tlf66會一筆為'Y',其它為NULL,故拿掉(o.rowno=1 OR o.tlf66 = 'X') 的限制
      PREPARE q761_wsaleamt04 FROM l_sql          
      EXECUTE q761_wsaleamt04

     #UPDATE axcq761_tmp SET wsaleamt=wsaleamt*-1 WHERE tlf907='1'  #MOD-D90159 add         #MOD-E50115 mark
      UPDATE axcq761_tmp SET wsaleamt=wsaleamt*-1 WHERE tlf907='1' AND oma00 NOT LIKE '1%'  #MOD-E50115 add
      ...
      END IF
      IF tm.b = '2' THEN   #非關係人
        #DELETE FROM axcq761_tmp WHERE occ37 ='N' #MOD-G20060 mark
         DELETE FROM axcq761_tmp WHERE occ37 ='Y' #MOD-G20060 add  查詢非關係人,所以要將關係人'Y'的刪除
      END IF    
...
#FUN-D10022----------------add-------------end-----------------------        
   ...
   IF tm.c = 'Y' THEN
      ...
      #FROM axcq761 WHERE tlf13 LIKE 'axmt%'       #MOD-F20003 mark
       FROM axcq761_tmp WHERE tlf13 LIKE 'axmt%'   #MOD-F20003 add
       
      SELECT SUM(tlf10),SUM(tlfc21)
        INTO l_num2,l_cost2
      #FROM axcq761 WHERE tlf13 LIKE 'axmt%' AND azf08 ='Y'      #MOD-F20003 mark
       FROM axcq761_tmp WHERE tlf13 LIKE 'axmt%' AND azf08 ='Y'  #MOD-F20003 add
       
      SELECT SUM(tlf10),SUM(tlfc21),SUM(wsaleamt) 
        INTO l_num3,l_cost3,l_wsaleamt2
      #FROM axcq761 WHERE tlf13 NOT LIKE 'axmt%'       #MOD-F20003 mark
       FROM axcq761_tmp WHERE tlf13 NOT LIKE 'axmt%'   #MOD-F20003 add
...
FUNCTION axcq761_table()
     ...
     CREATE TEMP TABLE axcq761_tmp(
                   ...
                   oma00         LIKE oma_file.oma00,        #MOD-E50115 add
...
DEFINE l_oma00         LIKE oma_file.oma00  #MOD-E50115 add 
   ...
   FOREACH tlf_curs INTO g_tlf_excel[g_cnt].*,g_tlfc[g_cnt].*,l_tlf13,l_tlf66,l_azf08,l_ogb01,l_ogb03,l_tlf907,l_oma00,l_rowno  #FUN-D10022 add l_tlf13,l_tlf66,l_azf08,l_rowno #FUN-CC0157 add ogb01,ogb03 #MOD-D90159 add l_tlf907 #MOD-E50115 add oma00
      IF SQLCA.sqlcode THEN
         CALL cl_err('foreach:',SQLCA.sqlcode,1)
         EXIT FOREACH
      END IF
