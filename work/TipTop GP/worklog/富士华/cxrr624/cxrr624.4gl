...
# Modify.........: # add by lixwz 20170726  cxrr624 中增加一列，名字叫not over due,然后按axrt300 中oma11 来算，未超过oma11 的放在not over due
...
#       l_time          LIKE type_file.chr8        #No.FUN-6A0095
         ...
         {}             num7      LIKE type_file.num20_6 ,#No.FUN-680123 DEC(20,6)
         	            num8      LIKE type_file.num20_6# add by lixwz 20170725
                    END RECORD
...
#FUN-A70084--mark--end
     ...
     IF g_ooz.ooz07 = 'N' THEN
        LET l_sql="SELECT oma14, gen02, oma15, ",
                  ...
                  "       oma56t-oma57,0,0,0,0,0,0,0,0,oma00 ",  # add by lixwz 20170726
     ...
     ELSE
        LET l_sql="SELECT oma14, gen02, oma15, ",
                  "       oma03, oma032,oma02, oma01,oma11,occ02,",
                  "       oma61,0,0,0,0,0,0,0,0,oma00 ",                  #No.A057  # add by lixwz 20170726
                 #FUN-A70084--mod--str--
                 ##" FROM ",l_plant CLIPPED,"oma_file,",l_plant CLIPPED,
                 ##"occ_file ,OUTER ", l_plant CLIPPED ,"gen_file",
                 #" FROM ",cl_get_target_table(l_p,'oma_file'),",",cl_get_target_table(l_p,'occ_file'),  #FUN-A50102
   ...
   # CALL cl_parse_qry_sql(l_sql,l_p) RETURNING l_sql          #FUN-A50102 #FUN-A70084
     ...
     FOREACH r624_curs1 INTO sr.*,l_oma00
      ...
      #CHI-830003--Add--End--#
       ...
       #    END CASE
       # mark by lixwz 20170727 e
       # add by lixwz 20170726 s
       LET l_day = tm.edate - sr.oma11
       CASE WHEN l_day   <=0 LET sr.num8 = sr.num
              WHEN l_day   <=tm.a1 LET sr.num1= sr.num
              WHEN l_day   <=tm.a2 LET sr.num2= sr.num
              WHEN l_day   <=tm.a3 LET sr.num3= sr.num
              WHEN l_day   <=tm.a4 LET sr.num4= sr.num
              WHEN l_day   <=tm.a5 LET sr.num5= sr.num
...
REPORT r624_rep(sr)
   DEFINE l_last_sw    LIKE type_file.chr1,   #No.FUN-680123 VARCHAR(1),
          l_amt1,l_amt2,l_amt3,l_amt4,l_amt5,l_amt6,l_amt7,l_amt8  LIKE type_file.num20_6,#No.FUN-680123    DEC(20,6),
...
#NO.FUN-580010--start
      ...
      LET g_zaa[42].zaa08='not over due'
      ...
      PRINT g_x[31],g_x[32],g_x[33],g_x[42],g_x[34],g_x[35],g_x[36],g_x[37],g_x[38],g_x[39],g_x[40],g_x[41]
      PRINT g_dash1
#No.FUN-580010--end
   AFTER GROUP OF sr.oma03
         ...
         LET l_amt8= GROUP SUM(sr.num8)# add by lixwz 20170725
         ...
         IF cl_null(l_amt8) THEN LET l_amt8 =0 END IF# add by lixwz 20170725

         LET l_amt_tot = l_amt1+l_amt2+l_amt3+l_amt4+l_amt5+l_amt6+l_amt7+l_amt8
#No.FUN-580010--start
         PRINT COLUMN g_c[31],sr.oma03,
               COLUMN g_c[32],sr.occ02[1,20],
               COLUMN g_c[33],sr.gen02,
               COLUMN g_c[42],cl_numfor(l_amt8,42,g_azi05),# add by lixwz 20170725
...
#No.FUN-580010--end
  ...
  #---------------------------MOD-C90207-----------------------(S)
   ...
   AFTER GROUP OF sr.oma14
         ...
         LET l_amt_tot = l_amt1 + l_amt2 + l_amt3 + l_amt4 + l_amt5 + l_amt6 + l_amt7 + l_amt8
         PRINT COLUMN g_c[42],cl_numfor(l_amt8,42,g_azi05),# add by lixwz 20170725
         	  COLUMN g_c[34],cl_numfor(l_amt1,34,g_azi05),
               COLUMN g_c[35],cl_numfor(l_amt2,35,g_azi05),
               COLUMN g_c[36],cl_numfor(l_amt3,36,g_azi05),
               COLUMN g_c[37],cl_numfor(l_amt4,37,g_azi05),
  ...
  #---------------------------MOD-C90207-----------------------(E)
   ON LAST ROW
         ...
         LET l_amt8= SUM(sr.num8)# add by lixwz 20170725
         IF cl_null(l_amt1) THEN LET l_amt1 =0 END IF
         IF cl_null(l_amt2) THEN LET l_amt2 =0 END IF
         IF cl_null(l_amt3) THEN LET l_amt3 =0 END IF
         IF cl_null(l_amt4) THEN LET l_amt4 =0 END IF
...
#No.FUN-580010--start
         ...
         PRINTX name=S1 g_x[18] CLIPPED,
               COLUMN  g_c[42],cl_numfor(l_amt8/l_amt_tot*100,42,2),# add by lixwz 20170725
