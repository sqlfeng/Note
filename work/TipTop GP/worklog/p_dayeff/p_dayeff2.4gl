...
FUNCTION gath_tm()
   ...
   OPEN WINDOW gath_w AT p_row,p_col WITH FORM "czz/42f/p_dayeff2" 
...
WHILE TRUE
   ...
   ON ACTION CONTROLP
      CASE
          WHEN INFIELD(tc_ser01)
            CALL cl_init_qry_var()
            LET g_qryparam.state = 'c'
            LET g_qryparam.form = "q_cust"
            CALL cl_create_qry() RETURNING g_qryparam.multiret
            DISPLAY g_qryparam.multiret TO tc_ser01
            NEXT FIELD tc_ser01
      END CASE
   ON ACTION locale
...
FUNCTION p_dayeff()
   DEFINE l_name VARCHAR(20),		# External(Disk) file name
          ...
          sr               RECORD order1 VARCHAR(20),
                                  order2 VARCHAR(20),
                                  order3 VARCHAR(20),
                                  tc_ser01    LIKE tc_ser_file.tc_ser01,
                                  year        INTEGER,
                                  month       INTEGER,
   ...
   DEFINE l_ser53_4 INTEGER
     ...
     LET l_sql = "SELECT DISTINCT '','','',",
                 " tc_ser01,Year(tc_ser06),MONTH(tc_ser06),count(*),0,0",             #add by leo 110901
                 " FROM tc_ser_file",
                 ...
                 " GROUP BY tc_ser01,Year(tc_ser06),Month(tc_ser06) "
     ...
     CALL cl_outnam('p_dayeff2') RETURNING l_name
...
#  ORDER BY sr.tc_ser06,sr.order1,sr.order2,sr.order3
  ORDER BY sr.tc_ser01,sr.year,sr.month
  FORMAT
   PAGE HEADER
   ...
   ON EVERY ROW
      ...
      PRINT COLUMN g_c[31],sr.tc_ser01 CLIPPED,
